...
function Retroactive() {
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>Retroactive Calculation of Productivity Loss</h2>

      {/* 操作区 */}
      <div style={styles.toolbar}>
        <div style={{ display: "flex", gap: "1rem" }}>
          <button onClick={handleReset} style={styles.resetBtn}>Reset</button>
          <button onClick={() => navigate("/proactive")} style={styles.switchBtn}>
            ← Switch to Proactive
          </button>
        </div>
      ...
      <div style={styles.tableWrapper}>
        <table style={styles.table}>
          <thead>
            <tr>
              <th style={styles.stickyHeader}>Weeks</th>
              {weeks.map((w, i) => (
                <th key={i} style={styles.weekHeader}>{w.replace('Week ', 'week ')}</th>
              ))}
            </tr>
          </thead>

          <tbody>
            {/* Planned Work Hours - 可编辑 */}
            <tr>
      ...
      <div style={styles.infoBox}>
        <p><strong>Note:</strong> This view shares the same data as Proactive Calculation. 
        Any changes made here or in the Proactive tool will automatically sync between both views.</p>
      </div>
    </div>
  );
}
